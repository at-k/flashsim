...
bool FtlImpl_Page_PC::increment_log_write_address(Event &event, bool *gc_required, bool bg_write)
{
	//printf("in ILWA\n");
	//fflush(stdout);
...
bool FtlImpl_Page_PC::allocate_new_block(Address requested_address, Event &event, bool *gc_required, bool bg_write)
{
	//printf("in ANB\n");
	//fflush(stdout);
	while(!bg_write && free_block_list.size() <= 5)
	{
...
enum status FtlImpl_Page_PC::write(Event &event, bool actual_time)
{
	...
	//printf("writing %d which is %d at ", event.get_logical_address(), actual_time == true);
	//log_write_address.print();
	...
	for(unsigned int i=0;i<PLANE_SIZE;i++)
	{
		...
		for(iter=allocated_block_list.begin();iter!=allocated_block_list.end();)
		{
			if((*iter).physical_address == cur_block_address)
			{
				...
				//printf("pushed to bg_cleaning_blocks %d\n", bg_cleaning_blocks.size());
				break;
			}
			else
				iter++;
