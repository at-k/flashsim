...
enum status Controller::issue(Event &event_list, bool remove)
{
	...
	for(cur = &event_list; cur != NULL; cur = cur -> get_next()){
		...
		if(cur -> get_event_type() == READ)
		{
			...
			printf("READ: %d %d %d %d %d %d %f ", remove, add.package, add.die, add.plane, add.block, add.page, cur->get_start_time());
		...
		{
			Address add = cur->get_address();
			printf("WRITE: %d %d %d %d %d %d %f ", remove, add.package, add.die, add.plane, add.block, add.page, cur->get_start_time());
			assert(cur -> get_address().valid > NONE);
		...
		{
			Address add = cur->get_address();
			printf("ERASE: %d %d %d %d %d %d %f ", remove, add.package, add.die, add.plane, add.block, add.page, cur->get_start_time());
			assert(cur -> get_address().valid > NONE);
			if(ssd.bus.lock(cur -> get_address().package, cur -> get_total_time(), BUS_CTRL_DELAY, *cur, remove) == FAILURE
