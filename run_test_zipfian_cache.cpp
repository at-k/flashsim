...
unsigned int get_random_lba(void)
{
  /* Inverse square.  */
  double exponent = ALPHA;
  /* Minimum value of distribution.  */
  double dist_min = MIN_LBA;
  /* Maximum value of distribution.  */
  double dist_max = MAX_LBA;

  double min_pow = pow (dist_min, exponent + 1);
  double max_pow = pow (dist_max, exponent + 1);

  return (unsigned int) pow(((max_pow - min_pow) * drand48() + min_pow), 1 / (exponent + 1));
}
...
int main(int argc, char **argv)
{
	...
	unsigned int total_read_count = 1500000, cur_read_count = 0;

	std::vector<unsigned int> cache;
	unsigned int cache_size = 2*1024*1024/4;
	...
	strcat(read_file_name, "zipfian_cache_read_");
	...
	strcat(write_file_name, "zipfian_cache_write_");
...
//		q_depth = 2*q_depth;
	...
	for(unsigned int i=0;i<q_depth;i++)
	{
		...
		{
			location = get_random_lba() - 1;
			std::vector<unsigned int>::iterator cache_entry = std::find(cache.begin(), cache.end(), location);
			while(cache_entry != cache.end())
			{
				cache.erase(cache_entry);
				cache.push_back(location);
				location = get_random_lba() - 1;
				cache_entry = std::find(cache.begin(), cache.end(), location);
			}
